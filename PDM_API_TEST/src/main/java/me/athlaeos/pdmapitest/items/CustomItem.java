package me.athlaeos.pdmapitest.items;
...
public class CustomItem extends ActiveItem {
    public CustomItem(){
        key = new NamespacedKey(Main.getInstance(), "pdm-custom_fuck");
        ...
        CooldownManager.getInstance().registerCustomItemCooldownMap(key.getKey());
    }
    ...
    public void execute(Event e) {
        ...
        if (PlayerCurseManager.getInstance().getCurse(p.getUniqueId()) < curseLimit){
            if (CooldownManager.getInstance().canPlayerUseItem(p.getUniqueId(), key.getKey()) || p.hasPermission("pdm.cancelitemcooldowns")) {
                ...
                if (!p.hasPermission("pdm.cancelitemcooldowns")) {
                    CooldownManager.getInstance().setItemCooldown(p.getUniqueId(), 5000, key.getKey());
                }

                p.sendMessage(Utils.chat("&cDEATH"));
                p.setHealth(0D);
            } else {
                p.sendMessage(Utils.chat(String.format("&cCooldown: %.2fs", (CooldownManager.getInstance().getItemCooldown(p.getUniqueId(), key.getKey()) / 1000D))));
            }
        } else {
